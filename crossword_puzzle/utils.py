...
from crossword_puzzle.constants import Paths, Colour
...
def _get_language_options() -> None:
    ...
    i: int = 0
    for locale in sorted([f.name for f in scandir(Paths.LOCALES_PATH) \
                            if f.is_dir()]):
...
def _load_cword_info(category: str,
    ...
    """
    path_ = path.join(Paths.LOCALES_PATH, language, "cwords", category, 
                        name, "info.json")
    if not path.exists(path_): # Fallback to base crossword info
        path_ = path.join(Paths.BASE_CWORDS_PATH, category, name, 
                            "info.json")
