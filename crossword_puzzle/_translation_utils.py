...
class LocaleTranslatorUtils:
    ...
    def update_msgstrs() -> None:
        ...
        for i, locale in enumerate(locales):
            messages: polib.POFile = polib.pofile(os.path.join(
            ...
            for entry in untranslated_entries:
                ...
                entry.msgstr = local_client.translate(
                    text, target_language=lang_codes[i], source_language="en", 
...
class CrosswordTranslatorUtils:
    ...
    def update_cword_translations() -> None:   
        ...
        for i, locale in enumerate(locales):
            ...
            for existing_category in [f.name for f in os.scandir(cwords_path) \
                                      if f.is_dir()]:
            ...
            for category, cwords in base_category_tree.items():
                ...
                for existing_cword in [f.name for f in os.scandir(category_path) \
                                      if f.is_dir()]:
                ...
                for cword in cwords: # Make and translate (if required) all 
                    ...
                    if os.listdir(cword_path): continue # Probably made 
                                                        # translations already idk

                    definitions, info = CrosswordTranslatorUtils._get_cword_data(
                                                                category, cword)
                    ...
                    print(f"(Locale: {locale}) - Wrote translations of "
                          f"{category}/{cword}")
    ...
    def _format_and_translate_cword_data(language: str,
        ...
        arrayified_definitions = np.array([pair for item in definitions.items() \
                                           for pair in item])
        ...
        info["translated_name"] = local_client.translate(
            info["name"], target_language=language, source_language="en", 
            format_="text")["translatedText"]
    ...
    def _translate_parts(definitions: List[np.array], 
        ...
        for part in definitions:
            translated_parts.append(local_client.translate(
                list(part), target_language=language, source_language="en", 
                format_="text"))
