...
class DefinitionsParser:
    """Serves methods to ensure a crossword's definitions are valid."""
    @staticmethod
    def _parse_definitions(definitions: Dict[str, str], 
        ...
        if any("\\" in word for word in definitions.keys()): # Escape character
                                                             # breaks regex
    ...
    def _format_definitions(definitions: Dict[str, str],
        ...
        everythingexcept the language characters from the words (the keys of 
        the definitions).
        """
        # Randomly ``word_count`` amount of definitions
        randomly_sampled_definitions = dict(sample(list(definitions.items()), 
                                                   word_count))
        ...
        formatted_definitions = {
            sub(CrosswordRestrictions.KEEP_LANGUAGES_PATTERN, 
